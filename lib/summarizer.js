...
// サービスアカウント優先、なければAPIキー
function createGenAI() {
  const saKeyPath = process.env.GEMINI_SERVICE_ACCOUNT_KEY;
  if (saKeyPath && fs.existsSync(saKeyPath)) {
    const credentials = JSON.parse(fs.readFileSync(saKeyPath, "utf8"));
    const projectId = process.env.GOOGLE_PROJECT_ID || credentials.project_id;
    if (projectId) {
      console.log(`Gemini auth: ServiceAccount (Vertex AI) project=${projectId}`);
  ...
  if (!process.env.GEMINI_API_KEY) {
    throw new Error("GEMINI_API_KEY or GEMINI_SERVICE_ACCOUNT_KEY が必要です");
  }
  console.log("Gemini auth: API Key");
...
async function callGemini(config, responseSchema, prompt, logLabel, opts = {}) {
  ...
  const model = aiCfg.model || "gemini-2.5-flash";
...
- 語尾は「〜じゃん」「〜くない？」「〜なんだけど」「てかさ〜」「〜まじ？」「〜だよね〜」「〜じゃんね」
- 驚き →「えぐ」「やば」「まって無理」「は？まじで？」
- 良い →「推せる〜」「神じゃん」「てかめっちゃ頑張ってる」「アガる〜」
- ヤバい →「それまじでやばくない？」「ちょっと待って？」「え、うそでしょ」「やばたにえん」
- Stuck →「詰んでるじゃん」「ヘルプ出しなよ〜」「放置はまじでよくないって〜」
- 名前は「〜ちゃん」「〜くん」呼び
- 「まじ」「てか」「めっちゃ」「ガチで」「それな」を多用せよ
